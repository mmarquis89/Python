...
def get_correlated_genes(df, seeds, correlation_cutoff, min_hits=1, exclude_max=0):
    ...
    for seed in seeds:
        ...
        if len(myValidatedHits) < min_hits:
            if seed == "SpikeIn1": print("SpikeIn1 was uncorrelated seed with len(validatedHits)=" + str(len(myValidatedHits)))
...
class ICIM:
    ...
    def step(self, parent, TFs=None, CSMs=None, exclude=None,
        ...
        if verbose:
            print("Found" + str(Y.shape[0]) + "genes")
        ...
        if verbose:
            print("Child populations" + str(X1.shape[1]) + str(X2.shape[1]))
